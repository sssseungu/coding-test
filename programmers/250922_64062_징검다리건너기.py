...
def can_cross(stones, k, mid):
    # 'mid'만큼의 인원이 건넌 후,
    # 그 다음 사람이 징검다리를 건널 수 있는가를 True OR False로 나타냄
    cnt = 0  # 'mid'만큼의 사람이 건넌 후 연속된 0의 개수 세기
    for stone in stones:
        if stone - mid <= 0:
            cnt += 1
            if cnt >= k:  # 연속된 0의 갯수가 k이상인 경우, 건널 수 없음
                return False
        else:
            cnt = 0
...
def solution(stones, k):
    ...
    #     Args:
    #         stones (list): 디딤돌에 적힌 숫자가 순서대로 담긴 배열
    #         k (int): 한 번에 건너뛸 수 있는 디딤돌의 최대 칸수

    #     Returns:
    #         cross_cnt (int): 징검다리를 건널 수 있는 최대 인원
    #     """

    left, right = 1, max(stones)
    cross_cnt = 0  # 징검다리를 건널 수 있는 최대 인원
